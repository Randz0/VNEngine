...
// These globals are dynamically allocated
...
void ConfigureDefaultDialogueBox() {
    ...
    loadedDialogueBox->xPos = (1920 - loadedDialogueBox->GetWidth()) * 0.5f;
...
void HandleDirective(std::string directive) {
    ...
    if (directive.find("fontSize=") == 1) {
        std::string newFontSize = directive.substr(10);

        FONTSIZE = std::stoi(newFontSize);
